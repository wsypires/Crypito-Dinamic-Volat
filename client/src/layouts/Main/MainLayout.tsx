...
export function MainLayout({ children }: Props) {
  ...
  const handleSidebarToggle = () => {
    ...
    // Dispatch event for chart resize after CSS transition completes
    // Using requestAnimationFrame for smoother coordination with browser rendering
    requestAnimationFrame(() => {
      setTimeout(() => {
        document.dispatchEvent(new Event('sidebar-toggle'));
      }, 320); // Slightly faster than CSS transition for smoother feel
    });
  };
